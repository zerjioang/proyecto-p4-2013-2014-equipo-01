...
public class HiloRetuits extends Thread {
	
	private TwitterService t;

	public HiloRetuits(TwitterService t) {
		this.t = t;
	}
	...
	public void run(){
		...
		try {
			p.setTextoMensajeInformativo("Actualizando retuits...");
			p.mostrarSpinWheelInformativa(false);
			p.mostrarMensajeInformativo();
			ResponseList<Status> listaTL = t.getRetweetsOfMe();
